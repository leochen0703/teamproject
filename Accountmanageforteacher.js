...
import './Accountmanageforteacher.css'; 

function Accountmanageforteacher() {
  ...
  const [addingUser, setAddingUser] = useState(false); // 控制是否顯示新增使用者表單
  ...
  useEffect(() => {
    // 從後端獲取使用者數據
    fetch('http://120.126.17.190:8000/api/v1/api/newuser/')
      ...
      <div className="background">
          ...
          {menuOpen && (
            <div className="menu-items">
              ...
              <button className="logout-btn" onClick={handleLogout}><h3>logout</h3></button>
                <Link to="/HomepageForAdmin" onClick={toggleMenu}>首頁</Link>
        ...
        <div className="management-container">
          <div className="title-container">
            <h2>管理</h2>
            <button className='add-user' onClick={() => setAddingUser(true)}>新增使用者</button>
          </div>
          <table className="unit-table">
            ...
            <tbody>
              ...
              {addingUser && (
                <tr>
                  <td><input type="text" name="name" value={newUser.name} onChange={handleInputChange} /></td>
                  <td><input type="text" name="type" value={newUser.type} onChange={handleInputChange} /></td>
                  <td><input type="text" name="contact" value={newUser.contact} onChange={handleInputChange} /></td>
                  <td><button onClick={handleAddUser}>新增</button></td>
                </tr>
              )}
            </tbody>
          </table>
...
export default Accountmanageforteacher;
