...
import './Chooseconnect.css';
import { Link,useNavigate,useLocation } from 'react-router-dom';

function Chooseconnect() {
  const [menuOpen, setMenuOpen] = useState(false);
  const [selectedConnection, setSelectedConnection] = useState(""); // 存储用户选择的连接方式
  ...
  const handleConnectionChange = (event) => {
    setSelectedConnection(event.target.value); // 更新使用者選擇連線方式
  };
    ...
    navigate(`/TaskNumber?connection=${selectedConnection}&cpu=${cpu}&threads=${threads}&memory=${memory}`);
  };
  useEffect(() => {
    const searchParams = new URLSearchParams(location.search);
    ...
    <div className="homepage-container">
      ...
      <div className="text-container">
        ...
        <form onSubmit={handleSubmit}>
            <br />
            <label>
            <input type="radio" name="Connect-Method" value="SSH" checked={selectedConnection === "SSH"} onChange={handleConnectionChange} />
            SSH
            </label>
            <br />
            <label>
            <input type="radio" name="Connect-Method" value="RDP" checked={selectedConnection === "RDP"} onChange={handleConnectionChange} />
            RDP
            </label>
            <br />
            <label>
            <input type="radio" name="Connect-Method" value="FTP" checked={selectedConnection === "FTP"} onChange={handleConnectionChange} />
            FTP
            </label>
            <br />
            <label>
            <input type="radio" name="Connect-Method" value="Webtop" checked={selectedConnection === "Webtop"} onChange={handleConnectionChange} />
            Webtop
            </label>
            <br />
            <label>
            <input type="radio" name="Connect-Method" value="WebIDE" checked={selectedConnection === "WebIDE"} onChange={handleConnectionChange} />
            WebIDE
            </label>
            <br />
