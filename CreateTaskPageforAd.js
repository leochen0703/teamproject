...
function CreateTaskPageforAd() {
    ...
    const handleSubmit = (event) => {
        event.preventDefault();
          ...
          // 檢查是否選擇了系統資源
        if (!selectedCPU || selectedCPU==="請選擇數量" || selectedThreads==="請選擇數量" || !selectedThreads || !selectedMemory) {
            alert('有資料未填寫');
            return;
        }

        console.log("Selected CPU cards:", selectedCPU);
        console.log("Selected CPU threads:", selectedThreads);
        console.log("Selected system memory:", selectedMemory);
        navigate(`/Chooseconnect?cpu=${selectedCPU}&threads=${selectedThreads}&memory=${selectedMemory}`);
    };
    useEffect(() => {
        ...
        // 在此处使用提取的数据，例如设置默认选项
        setSelectedCPU(cpu || "請選擇數量");
        setSelectedThreads(threads || "請選擇數量");
    ...
    const handleExampleChange = (e) => {
        ...
        // 根据选择的预设範例自动设置系统资源
        if (e.target.value === "MATLAB") {
            setSelectedCPU(2);
            setSelectedThreads(4);
            setSelectedMemory('16GB');
        }
    };
            ...
            <div className="background">
                ...
                <div className="text-container">
                    ...
                    <form onSubmit={handleSubmit}>
                        <label>
                            CPU cards per container number:
                            <select name="CPU_cards" value={selectedCPU} onChange={(e) => setSelectedCPU(parseInt(e.target.value))}>
                            <option value="">請選擇數量</option>
                            <option value="1">1</option>
                            <option value="2">2</option>
                            <option value="3">3</option>
                            </select>
                            <span className="note">備註:如需兩顆以上須連絡團隊</span>
                        </label>
                        <br />
                        <label>
                            CPU threads per container number:
                            <select name="CPU_threads" value={selectedThreads} onChange={(e) => setSelectedThreads(parseInt(e.target.value))}>
                            <option value="">請選擇數量</option>
                            <option value="1">1</option>
                            <option value="2">2</option>
                        ...
                        <br />
                        <label>
                            System memory per container number:
                            <input type="text" name="System_memory" value={selectedMemory} onChange={(e) => setSelectedMemory(e.target.value)} placeholder="請選擇8~80" />GB
                        </label>
                        <br />
                    ...
                    <div className="description-container">
                        <h2>套用預設範例</h2>
                        <p>若不知如何設定，可根據不同用途，有預先設定好的參數供使用</p>
                        <select name="Example" value={selectedExample} onChange={handleExampleChange}>
...
export default CreateTaskPageforAd;
